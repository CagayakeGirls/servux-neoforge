...
public class PermissionsUtil
{
	...
	{
		return PermissionsHelper.require(node, PermissionLevel.byId(Mth.clamp(level, 0, PermissionLevel.OWNERS.id())));
	}

	public static boolean check(Entity entity, @NotNull String node, int level)
	{
		return PermissionsHelper.check(entity, node, PermissionLevel.byId(Mth.clamp(level, 0, PermissionLevel.OWNERS.id())));
	}
}
